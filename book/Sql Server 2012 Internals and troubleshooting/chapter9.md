...
데이터 수집
* 데이터를 수집하는 것은 문제를 명확히 짚어내기 위해 반복되어야 하는 작업이다. 데이터를 수집하는 것 자체가 근본 문제를 명확히 하기위한 단계이다. 
* 문제 발생시 데이터를 수집하는 방식은 Top-down 방식, Devide-and-conquer 방식이 유용하다. 

데이터 수집의 이해
* 문제가 발생한 데이터를 수집할 때, 자주 일어나는 경우와 그렇지 않은 경우로 구분해서 수집해야 한다. 예측할 수 있는 패턴의 경우 데이터 수집에 긴 시간이 걸리거나 필요한 데이터를 모두 수집할 수있다. 그렇지 않은 경우 다른 전략이 필요하다. black box circular trace(같은 문제 발생 시 덮어쓰기됨)로 수집한다.

Tool and Utilities
* PerfMon - Windows에서 제공(전반적인 하드웨어 및 sql의 모니터링도 가능)
* Profiler - Sql server에서 제공(데이터베이스 엔진의 statemnet-level에서 모니터링가능)
* Xevents 
* PSSDiag
* SQLDiag
* Event Logs - Windows에서 제공
* Application logs
* User dumps
* NetMon - 네트워크 이슈를 확인 가능
* CMS
* Management Data Warehouse
* Policy-Based Management

데이터 분석
* 데이터 수집 후 문제의 근본원인을 파악하기 위해 두 번째로 반복되어야 할 작업이다. 이 부분에서 문제 해결의 성공 기준을 어디로 잡아야할 지 결정하는 단계이다. 

문제 해결의 유효성 검사 및 수행
* 문제해결 방법이 명확해 졌다고 바로 수행하는 것이 아니라 유효성 검사를 해야 한다. 테스팅 환경에서 먼저 유효성을 확인해서 문제를 해결하는 결과와 맞게 나오는지 확인을 하는 것이 필요하다.
* 테스트를 진행할 때도 여러 옵션을 적용해야 해도 하나씩 적용을 하면서 테스트하고, 기록해야 한다. 나중에 롤백을 대비하기 위함이고, 테스트 도중 문제가 발생해도 어느 부분에서 문제가 발생했는지 명확히 파악을 할 수 있다.

문제해결 수행
* 마지막으로 해결 방법대로 진행하는 것이다. 변경된 사항과 영향 범위에 대한 문서화와 보장이 필요하다. 적용 전과 후에 대한 기록을 남겨야 한다. 문제해결의 성공 기준에 맞게 적용되었는지도 확인이 필요하다.

SUMMARY
* 문제가 발생했을 때 각각의 경우에 대한 해결방법에 대한 시나리오를 가지고 있는 것이 좋다. 
* 문제의 정의, 어느 부분에서 문제가 발생을 했는지, 문제의 영향범위가 무엇인지, 문제를 쪼개어서 확인하고, 데이터를 수집하고 분석하여 근본원인을 찾고, 해결방법을 적용하기 전 방법이 정말 명확한지에 대한 테스트를 적용하여 원하는 결과가 나오는 것을 확인하고 적용한다.
